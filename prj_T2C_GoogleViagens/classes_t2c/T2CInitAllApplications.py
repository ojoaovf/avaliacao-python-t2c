...
from prj_T2C_GoogleViagens.classes.ExtrairDadosMundiais import ExtrairDadosMundiais
...
class T2CInitAllApplications:
    ...
    def add_to_queue(self):
        ...
        Parâmetros:
        """
        var_clssExtrairDadosMundiais = ExtrairDadosMundiais(arg_dictConfig=self.var_dictConfig, arg_clssMaestro=self.var_clssMaestro)
        var_listPaisesExtraidos = var_clssExtrairDadosMundiais.execute()

        for var_intItem in var_listPaisesExtraidos:
            self.var_clssSqliteQueue.insert_new_queue_item(arg_strReferencia = [var_intItem], arg_listInfAdicional = [var_intItem])
